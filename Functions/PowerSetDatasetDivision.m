function [split, featuresAccuracy,featuresAccuracyPlus,featuresAccuracyMinus,ResultingCentroids] = PowerSetDatasetDivision(Dataset,MaxFeatures,K)
...
%             TrainMinus=minus(:,trainIndexMinus );
            ...
            Centroids{kk} = l2SparseCentroids(plus(:,trainIndexPlus ),minus(:,trainIndexMinus ),ff);
            ...
            accuracyPlus(kk)=sum(CostPlus>=0)/length(CostPlus);
            accuracyMinus(kk)=sum(CostMinus<=0)/length(CostMinus);
            clear CostMinus
            clear CostPlus
        ...
        AccuracyPlus{ii}(1,ff)=mean(accuracyPlus);
        AccuracyMinus{ii}(1,ff)=mean(accuracyMinus);
